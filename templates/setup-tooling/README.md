...
6. Do not commit dirty submodule gitlinks after `--remote` unless you intend to freeze a pin. CI should keep `submodules: recursive` (recorded pin), not `--remote`. Only `doqs` and `.agents` should show as dirty after a helper run; anything under `modules/` showing as dirty means the helper is an old copy.
